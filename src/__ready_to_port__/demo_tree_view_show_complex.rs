...
pub fn DemoTreeViewShowComplex() -> impl IntoView {
    view! {
        <div class="flex gap-4 w-full max-w-4xl">
            ...
            <div class="overflow-hidden flex-1 rounded-md border bg-card border-border">
                <div id="content-display" class="h-full">
                    <div class="p-4 border-b bg-muted/30">
                        <h3 class="text-lg font-semibold">main.rs</h3>
        ...
        <li data-name="File" class="flex flex-row -ml-4">
            <input
                ...
                on:change=move |_| {
                    ...
                    {
                        let file_extension = name.split('.').last().unwrap_or("");
                            ...
                            .set_inner_html(
                                &format!(
                                    r#"<div class="h-full flex flex-col">
                                        <div class="p-4 border-b bg-muted/30 flex-shrink-0">
                                            <h3 class="font-semibold text-lg">{}</h3>
                                            <p class="text-sm text-muted-foreground mt-1">Rust source file</p>
                                        </div>
                                        <div class="flex-1 overflow-auto">
                                            <pre class="p-4 text-sm font-mono bg-muted/10 h-full overflow-auto"><code class="language-{}">{}</code></pre>
                                        </div>
                                    </div>"#,
                                    name,
                                    file_extension,
